...
Adotar a opção A na Sprint 2. A migration da Sprint 2.1 implementou originalmente o trigger transacional com `role = vendedor`, valor agora substituído pela decisão `seller`. Como a migration não foi aplicada segundo a evidência disponível, ela e seus testes devem ser reconciliados antes da aplicação. O nome opcional continua limitado a `raw_user_meta_data.full_name` ou, como fallback, `raw_user_meta_data.name`; metadados de autorização nunca são lidos. Observabilidade e reconciliação continuam obrigatórias. Se testes reais mostrarem incompatibilidade com o fluxo de convite, a troca para B exige atualizar este documento e o ADR antes da implementação.
...
- **PENDENTE:** atualizar a migration e os testes SQL versionados de `vendedor` para `seller` antes de qualquer aplicação;
