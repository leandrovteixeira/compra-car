...
function emptyCategories():readonly SellerCategoryScore[]{return Object.freeze<SellerCategoryScore[]>([
 ...MONETARY_CATEGORIES.map(c=>({key:c,label:displayCategory(c),score:null})),
...
export async function loadSellerModelScore(productId:number|null,radius:SellerScoreRadius):Promise<SellerModelScoreResult>{
 ...
 const categories=Object.freeze<SellerCategoryScore[]>([...monetaryScores,{key:'Espaco + Carga',label:'Espaço + Carga',score:spaceScore}]);const valid=categories.flatMap(c=>c.score===null?[]:[c.score]);const overallScore=valid.length?valid.reduce((a,b)=>a+b,0)/valid.length:null;
