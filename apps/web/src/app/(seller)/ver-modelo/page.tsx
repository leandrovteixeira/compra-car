...
return <main className="min-h-[calc(100dvh-var(--app-topbar-height))] bg-background px-3 py-4 text-text-primary sm:px-6 lg:px-8"><div className="mx-auto w-full max-w-[100rem]">
 ...
 <section className="mt-4 min-w-0">
  ...
  <div className="mt-3 grid gap-3 xl:grid-cols-[11.5rem_minmax(0,1fr)_25rem] xl:items-stretch">
   <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 xl:grid-rows-2">
    <div className="flex min-h-36 flex-col justify-between rounded-xl border border-border bg-surface-muted p-4 xl:min-h-0"><span className="text-sm font-semibold uppercase tracking-wide text-text-secondary">Score</span><strong className="self-center text-5xl font-semibold leading-none xl:text-6xl">{score(result.overallScore)}</strong><span aria-hidden="true" className="h-4" /></div>
    <div className="flex min-h-36 flex-col justify-between rounded-xl border border-border bg-surface-muted p-4 xl:min-h-0"><span className="text-sm font-semibold uppercase tracking-wide text-text-secondary">Modelos</span><strong className="self-center text-5xl font-semibold leading-none xl:text-6xl">{result.peerCount}</strong><span className="text-center text-[0.6875rem] text-text-muted">no raio de ±{radius}%</span></div>
   </div>
   <SellerModelRadar categories={result.categories}/>
   <div className="ui-surface !p-4"><h2 className="text-base font-semibold">Notas por categoria</h2><div className="mt-3 grid grid-cols-2 gap-x-5">{result.categories.map(c=><div className="flex items-center justify-between gap-3 border-b border-border py-2" key={c.key}><span className="text-xs text-text-secondary">{c.label}</span><span className="text-sm font-semibold">{score(c.score)}</span></div>)}</div><p className="mt-3 text-[0.6875rem] leading-4 text-text-muted">10 = maior valor observado na categoria dentro do raio. Espaço + Carga usa dimensões físicas.</p></div>
  </div>
 </section>}
</div></main>}
